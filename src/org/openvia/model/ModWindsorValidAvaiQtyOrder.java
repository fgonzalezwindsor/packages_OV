						...
						&& oLine.getM_Product().getProductType().compareTo("I") == 0)
				{
					...
					{	
						...
						else
						{
							...
							// Se comenta por solicitud de Fernando 07-10-2020
//							if( order.getC_BPartner_Location_ID()  == 1011338)
//							{
...
//									BigDecimal aux3 = DB.getSQLValueBD(po.get_TrxName(), "SELECT qtyavailableofb(mp.M_Product_ID,1000028) FROM M_Product mp WHERE mp.M_Product_ID = "+oLine.getM_Product_ID());
//									if(aux3 == null)
//										aux3 = Env.ZERO;
//									qtyAvai = qtyAvai.add(aux3);
//								}
...
//							}
							...
							else if(order.getM_Warehouse_ID() == 1000027)
							{
								...
								BigDecimal aux2 = DB.getSQLValueBD(po.get_TrxName(), "SELECT qtyavailableofb(mp.M_Product_ID,1000028) FROM M_Product mp WHERE mp.M_Product_ID = "+oLine.getM_Product_ID());
								if(aux2 == null)
									aux2 = Env.ZERO;							
								if(aux2.subtract(oLine.getQtyOrdered()).compareTo(Env.ZERO) < 0)								
									return "ERROR: Stock Insuficiente: "+aux2.intValue()+". Stock en otras bodegas:"+qtyAvai.intValue();
