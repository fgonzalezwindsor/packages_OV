...
//								}
	...
	@SuppressWarnings("unchecked")
	@Override
	public boolean apiPutCliente(IClientesModel cli) {
		boolean ret = false;
		HttpClient httpClient = new HttpClient();
		String url = "http://190.215.113.91/InaCatalogAPI/api/iClientes?empresa=" + cli.getCodEmpresa() + "&codcliente=" + cli.getCodCliente();
	...
	public List<IClientesModel> clientesNuevos() {
		...
		IClientesModel cliente = null;
		try {
			jsonArray = readJsonArrayFromUrl2("http://190.215.113.91/InaCatalogAPI/api/iClientes?empresa=1&flaNvoCliente=2&flaExpCliente=0");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public JSONArray readJsonArrayFromUrl2(String url) {
		JSONArray jsonArray = null;
		try {
