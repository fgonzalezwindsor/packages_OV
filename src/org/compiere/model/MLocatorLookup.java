...
public final class MLocatorLookup extends Lookup implements Serializable
{
	...
	class Loader extends Thread implements Serializable
	{
		...
		//	log.config("MLocatorLookup Loader.run " + m_AD_Column_ID);
			...
			StringBuffer sql = new StringBuffer("SELECT M_Locator_ID, AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy, Value||CASE WHEN (SELECT COUNT(M_Product_ID) FROM M_Storage WHERE M_Storage.M_Locator_ID = M_Locator.M_Locator_ID)=0 THEN ' - Vacía' END Value, M_WareHouse_ID, PriorityNo, IsDefault, X, Y, Z, IsPatio, Description, NoUseInPicking FROM M_Locator ")
