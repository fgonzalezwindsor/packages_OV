...
public class MOrder extends X_C_Order implements DocAction
{
	...
	public int copyLinesFrom (MOrder otherOrder, boolean counter, boolean copyASI)
	{
		...
		for (int i = 0; i < fromLines.length; i++)
		{
			...
			if (line.save(get_TrxName()))
				count++;
			//	Cross Link
			//ininoles copiar referencia en la linea creada
			try{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//faaguilar OFB validacion existencia Begin
		if (getDocumentNo() != null && getDocumentNo().startsWith("<")==false && getDocumentNo().endsWith(">")==false)
		if(isSOTrx())
		{
			int existe=0;
			String fixDocumentNo=getDocumentNo();
			fixDocumentNo=fixDocumentNo.replace('>',' ');
			fixDocumentNo=fixDocumentNo.replace('<',' ');
			fixDocumentNo=fixDocumentNo.trim();
			log.info("*Documentno: "+fixDocumentNo);
			existe=DB.getSQLValue(null,"select count(1) from c_order where docstatus IN ('CO','DR','IP') and issotrx='Y' "
						+ " and DocumentNo='"+fixDocumentNo +"' and AD_Org_ID="+getAD_Org_ID() +" and C_Order_ID<>"+getC_Order_ID() + " and C_DocTypeTarget_ID="+getC_DocTypeTarget_ID());
					
						if(existe>0)
						{
							log.saveError("Error", "Ya existe una Orden para esta Organizacion con el mismo Numero");
							m_processMsg = "Ya existe una Orden para esta Organizacion con el mismo Numero";
							return false;
						}
			
		}
		//faaguilar validacion existencia END
		...
		//	Warehouse Org
		/***faaguilar OFB codigo original comentado para que no valide
		 * la Org de la orden vs Org Warehouse
		if (newRecord 
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		// 1000574: OCM PRD
		// 1000571: OCM DES
		if (is_ValueChanged("DateinPort") && getC_DocTypeTarget_ID() == 1000574) {
			String dateInPortOld = get_ValueOld("DateinPort")==null?"":get_ValueOld("DateinPort").toString().substring(0, 10);
			String dateInPort = get_Value("DateinPort")==null?"":get_Value("DateinPort").toString().substring(0, 10);
			if (get_Value("DateinPort")==null && get_ValueOld("DateinPort")==null) {
				//String sql = "UPDATE C_Order SET dateinport = null, ov_olddateinport = null WHERE C_Order_ID = " + getC_Order_ID();
				String sql = "UPDATE AD_CHANGELOG SET NEWVALUE = null, OLDVALUE = null WHERE AD_TABLE_ID = 259 AND AD_COLUMN_ID = 1006796 AND RECORD_ID = " + getC_Order_ID();
				DB.executeUpdate(sql, null);
			} else if (get_Value("DateinPort")==null && get_ValueOld("DateinPort")!=null) {
				//String sql = "UPDATE C_Order SET dateinport = null, ov_olddateinport = TO_DATE('" + dateInPortOld + "', 'yyyy-MM-dd') WHERE C_Order_ID = " + getC_Order_ID();
				String sql = "UPDATE AD_CHANGELOG SET NEWVALUE = null, OLDVALUE = '" + dateInPortOld + "' WHERE AD_TABLE_ID = 259 AND AD_COLUMN_ID = 1006796 AND RECORD_ID = " + getC_Order_ID();
				DB.executeUpdate(sql, null);
			} else if (get_ValueOld("DateinPort")==null && get_Value("DateinPort")!=null) {
				//String sql = "UPDATE C_Order SET dateinport = TO_DATE('" + dateInPort + "', 'yyyy-MM-dd'), ov_olddateinport = null WHERE C_Order_ID = " + getC_Order_ID();
				String sql = "UPDATE AD_CHANGELOG SET NEWVALUE = '" + dateInPort + "', OLDVALUE = null WHERE AD_TABLE_ID = 259 AND AD_COLUMN_ID = 1006796 AND RECORD_ID = " + getC_Order_ID();
				DB.executeUpdate(sql, null);
			} else {
				//String sql = "UPDATE C_Order SET dateinport = TO_DATE('" + dateInPort + "', 'yyyy-MM-dd'), ov_olddateinport = TO_DATE('" + dateInPortOld + "', 'yyyy-MM-dd') WHERE C_Order_ID = " + getC_Order_ID();
				String sql = "UPDATE AD_CHANGELOG SET NEWVALUE = '" + dateInPort + "', OLDVALUE = '" + dateInPortOld + "' WHERE AD_TABLE_ID = 259 AND AD_COLUMN_ID = 1006796 AND RECORD_ID = " + getC_Order_ID();
				DB.executeUpdate(sql, null);
			}
			//int p_AD_PrintFormat_ID = 1001708; // 1001708: DES
			int p_AD_PrintFormat_ID = 1001797; // 1001797: PRD
			SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy");
			SimpleDateFormat dfOld = new SimpleDateFormat("yyyy-MM-dd");
			MClient M_Client = new MClient(Env.getCtx(),get_TrxName());
			EMail email = null;
			try {
				email = M_Client.createEMail("venta@comercialwindsor", "Cambio de Fecha en Puerto Orden de Compra Madre " + getDocumentNo() + " " + new Timestamp(System.currentTimeMillis()), "Nueva Fecha en Puerto: " + (get_Value("DateinPort")==null?"":df.format(dfOld.parse(get_Value("DateinPort").toString().substring(0, 10)))) + "<br/> Fecha en Puerto Anterior: " + (get_ValueOld("DateinPort")==null?"":df.format(dfOld.parse(get_ValueOld("DateinPort").toString().substring(0, 10)))), true);
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			email.addCc("dorta@comercialwindsor.cl");
			email.addCc("agrande@comercialwindsor.cl");
			email.addCc("arondon@comercialwindsor.cl");
			...
			MQuery query = new MQuery("C_Order_ID");
			query.addRestriction("C_Order_ID", MQuery.EQUAL, new Integer(getC_Order_ID()));
			...
			if (!Ini.isClient())
				cierreLine = new File(getPDFFileName(documentDir, getC_Order_ID()));
	...
	public String prepareIt()
	{
		...
		/**
		 * faaguilar OFB
		 * validacion al completar orden de compra
		 * verificando monto del rol, en base a funcionalidad especial (fuera del workflow)*/
		//faaguilar OFB alert message amount
		if(!isSOTrx())
		{
			MRole myrole = new MRole(getCtx(),Env.getAD_Role_ID(getCtx()),get_TrxName() );
			Boolean useWorkflow="Y".equals(DB.getSQLValueString(get_TrxName(), "select Useworkflow from AD_Client where AD_Client_ID="+Env.getAD_Client_ID(getCtx())));
			BigDecimal invAmt = MConversionRate.convertBase(getCtx(), getGrandTotal(),	//	CM adjusted 
			...
			if(myrole.getAmtApproval().compareTo(invAmt)<0 && myrole.getAmtApproval().compareTo(Env.ZERO)>0 && !useWorkflow)
			{
				ADialog.warn(2, new ConfirmPanel(), "Solo puede Aprobar documentos hasta un monto de "+myrole.getAmtApproval().intValue());
				setProcessed(true);
				DB.executeUpdate("Update C_Order set Processed='Y' where C_order_ID="+getC_Order_ID(), get_TrxName());
				return DocAction.STATUS_WaitingConfirmation;
			}
		}
		//faaguilar OFB alert message amount END
		...
		//faaguilar OFB validacion stock QDC begin
		if(getC_DocTypeTarget().getDocSubTypeSO()==null || (getC_DocTypeTarget().getDocSubTypeSO()!=null && !getC_DocTypeTarget().getDocSubTypeSO().equals(DocSubTypeSO_RMA) ) )
		{
			try {
					MClient client = MClient.get(getCtx(), getAD_Client_ID());
					if(isSOTrx() && client.getName().equals("QDC"))
					{
						for(MOrderLine line:getLines())
						{
							BigDecimal onHand = DB.getSQLValueBD(get_TrxName(), "select SUM(s.qtyonhand) from M_Storage s " +
									"Inner join M_Locator l on (s.M_Locator_ID=l.M_Locator_ID) where l.M_Warehouse_ID=? and s.M_Product_ID=? and l.isactive='Y' ", getM_Warehouse_ID(),line.getM_Product_ID());
							
							BigDecimal difValidacion = line.getQtyOrdered().subtract(line.getQtyDelivered()); 			
							if(difValidacion.compareTo(onHand)>0 && get_ValueAsString("SignatureStock").equals("false") )
							{
										  m_processMsg= "Superado el Stock del producto : "+line.getM_Product().getName()+ ", solo se poseen :"+onHand;
										  return DocAction.STATUS_Invalid;
							}
						}
		...
		//faaguilar OFB validacion stock QDC end
				

		//	Lines
		if(!OFBForward.NoExplodeBOMOrder())
		{
			if (explodeBOM())
		...
		/**
		 * faaguilar OFB valida que exista tipo de cambio*/
		//faaguilar Begin
		BigDecimal grandTotalv = MConversionRate.convertBase(getCtx(), 
				getGrandTotal(), getC_Currency_ID(), getDateOrdered(), 
				getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
			
			log.info("**grandTotal:"+grandTotalv);
			
			if (grandTotalv == null)
			{
				m_processMsg = "No existe un tipo de cambio valido para la moneda C_Currency_ID=" + getC_Currency_ID()
				+ " to base C_Currency_ID=" + MClient.get(Env.getCtx()).getC_Currency_ID();
				return DocAction.STATUS_Invalid;
			}
			//faaguilar END
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (product != null) 
			{
				if (product.isStocked())
				{
					...
					if(existReservationTable())//faaguilar OFB
						OFBReservation(line.getM_Warehouse_ID(),line.getM_Product_ID(),ordered,reserved);//faaguilar OFB
					...
					//faaguilar OFB set locator begin
					try{
						line.set_Value("M_Locator_ID", M_Locator_ID);
					}
					catch(Exception e){
						log.severe("OrderLine - no field M_Locator_ID");
					}
					//faaguilar OFB set locator end
	...
	public String completeIt()
	{
		...
		if (MDocType.DOCSUBTYPESO_WarehouseOrder.equals(DocSubTypeSO)	//	(W)illCall(P)ickup	
		...
		/**
		 * faaguilar OFB 
		 * actualiza balance de socio de negocio*/
//		Update BP Statistics
		...
		if (isSOTrx())
		{
			...
			if (bp.getFirstSale() == null)
				bp.setFirstSale(getDateOrdered());
			...
			//
			log.fine("GrandTotal=" + getGrandTotal() + "(" + invAmt
		...
		/**
		 * faaguilar OFB
		 * actualizacion de requisition relacionada 
		 * al completar*/
		//ininoles ahora se actualizara la linea de solicitud al guardar la linea de la OC
		//UpdateRequisition("CO"); //faaguilar OFB 
		
		updateBudget(); //faaguilar OFB
		/**
		 * faaguilar OFB
		 * para ordenes de compra si un usuario la autoriza queda guardado que usuario la aprobo*/
		try{
			this.set_Value("AproUser_ID", Env.getAD_User_ID(getCtx()));
		}
		catch(Exception e){}
		//faaguilar END
	...
	public boolean voidIt()
	{
		...
		/**
		 * faaguilar OFB
		 * valida antes de anular que no existan documentos invoice o inout
		 * completados
		 * */
		//faaguilar OFB validation before void links documents
		MDocType dt = MDocType.get(getCtx(), getC_DocType_ID());
		String DocSubTypeSO = dt.getDocSubTypeSO();
		...
		if (MDocType.DOCSUBTYPESO_StandardOrder.equals(DocSubTypeSO) || getC_DocTypeTarget().getDocBaseType().compareToIgnoreCase("POO")==0)
		{
			String mysql="select count(1) from C_InvoiceLine ivl "+
						...
						"where ivl.C_OrderLine_ID IN (select C_OrderLine_ID from C_OrderLine where C_Order_ID="+getC_Order_ID()+") "+
						"and iv.DocStatus IN ('CO','CL','IP')";
						
			int count=DB.getSQLValue(get_TrxName(),mysql);
			if(count>0)			
			{
				m_processMsg = "Esta Orden posee una Factura Valida Relacionada, no puede ser Anulada.";
				return false;
			}
			
			mysql="select count(1) from M_InOutLine ivl "+
			"inner join M_InOut iv on (ivl.M_InOut_ID=iv.M_InOut_ID) "+
			"where ivl.C_OrderLine_ID IN (select C_OrderLine_ID from C_OrderLine where C_Order_ID="+getC_Order_ID()+") "+
			"and iv.DocStatus IN ('CO','CL','IP')";
			count=DB.getSQLValue(get_TrxName(),mysql);
			if(count>0)			
			{
				
				m_processMsg = "Esta Orden posee un Movimiento Valido Relacionado, no puede ser Anulada.";
				return false;
			}
		}
		//faaguilar OFB validation before void links documents END
		//ininoles nuevo calculo de qtyused en lineas de orden
		//UpdateRequisition("VO"); //faaguilar OFB
		
		/**
		 * faaguilar OFB 
		 * actualiza balance de socio de negocio begin*/
//		Update BP Statistics
		MBPartner bp = new MBPartner (getCtx(), getC_BPartner_ID(), get_TrxName());
		//	Update total revenue and balance / credit limit (reversed on AllocationLine.processIt)
		BigDecimal invAmt = MConversionRate.convertBase(getCtx(), getGrandTotal(),	//	CM adjusted
		...
		/**
		 * faaguilar OFB 
		 * actualiza balance de socio de negocio end*/
...
//		Update BP Statistics
		...
		//modificacion para UNAB NO USAR EN FUENTES OFICIALES ininoles
		try
		{				
			
			if (OFBForward.OrderUNAB())
			{
				...
				for (int i = 0; i < lines.length; i++)				
				{
					...
					X_T_PRESUPUESTO pres = new X_T_PRESUPUESTO(this.getCtx(), oLine.get_ValueAsInt("T_PRESUPUESTO_ID"), this.get_TrxName());
					BigDecimal amtDevUNAB = DB.getSQLValueBD(this.get_TrxName(), "select sum(amt) from T_BudgetDistributionOrder where isactive='Y' and contributiontype = '01' and C_OrderLine_ID="+ oLine.get_ID());
					BigDecimal amtDevMine = DB.getSQLValueBD(this.get_TrxName(), "select sum(amt) from T_BudgetDistributionOrder where isactive='Y' and contributiontype = '02' and C_OrderLine_ID="+ oLine.get_ID());
					...
					DB.executeUpdate("UPDATE T_BudgetDistribution set Balance2 = Balance2 - "+amtDevUNAB+" WHERE T_PRESUPUESTO_ID = "+pres.get_ID()+" AND contributiontype = '01' ", this.get_TrxName());
					DB.executeUpdate("UPDATE T_BudgetDistribution set Balance2 = Balance2 - "+amtDevMine+" WHERE T_PRESUPUESTO_ID = "+pres.get_ID()+" AND contributiontype = '02' ", this.get_TrxName());
	...
	/**
	 * faaguilar OFB
	 * actualiza ya sea al anular o completar
	 * una requisition relaciona
	 * con la orden*/
	public void UpdateRequisition(String status)
	{
		MOrderLine[] lines= getLines(true, null);
		for (int i = 0; i < lines.length; i++)
		{
			if(lines[i].get_ValueAsInt("M_RequisitionLine_ID")>0){
				MRequisitionLine line=new MRequisitionLine(getCtx(), lines[i].get_ValueAsInt("M_RequisitionLine_ID"), get_TrxName());
				if(status.equals("CO"))
					line.set_Value("QtyUsed",((BigDecimal)line.get_Value("QtyUsed")).add(lines[i].getQtyOrdered()));
				else
					line.set_Value("QtyUsed",((BigDecimal)line.get_Value("QtyUsed")).subtract(lines[i].getQtyOrdered()));
				line.save();
			}
		}
	}
	...
	/**
	 * faaguilar OFB
	 * actualiza el presupuesto usado real
		...
		if("Y".equals(DB.getSQLValueString(get_TrxName(), "select UseBudget from AD_Client where AD_Client_ID="+getAD_Client_ID())) && !isSOTrx()){
			...
			for(MOrderLine line:lines){
				if(line.get_ValueAsInt("M_RequisitionLine_ID")>0){
					MRequisitionLine rline = new  MRequisitionLine(getCtx(), line.get_ValueAsInt("M_RequisitionLine_ID"),get_TrxName());
					
					MPeriod period= MPeriod.get(getCtx(), rline.getParent().getDateDoc());
					BigDecimal monto = MConversionRate.convertBase(getCtx(), line.getLineNetAmt(), getM_PriceList().getC_Currency_ID(), getDateOrdered(), 0, getAD_Client_ID(), line.getAD_Org_ID());
					
					if(!rline.get_ValueAsBoolean("IsCorrected")){
						DB.executeUpdate("Update m_product_category_Budget set BudgetUsed=BudgetUsed - "+ monto.doubleValue()
								+" where m_product_category_id="+ line.getM_Product().getM_Product_Category_ID() +" And AD_Org_ID="+ rline.getAD_Org_ID()+ " and c_period_ID="+period.getC_Period_ID(), get_TrxName());
					
						rline.set_CustomColumn("IsCorrected", true);
						rline.save();
					}
	...
	/**
	 * faaguilar OFB
	 * libera presupuesto al anular la orden*/
