...
public class CreateFromShipment extends CreateFrom 
{
	...
	protected ArrayList<KeyNamePair> loadLlegadaData (int C_BPartner_ID)
	{
		...
		.append(" FROM OV_Llegada s, C_Order o, OV_LlegadaLine sl"
				+ " WHERE s.C_Order_ID = o.C_Order_ID"
				+ " AND s.OV_Llegada_ID = sl.OV_Llegada_ID"
				+ " AND s.C_BPartner_ID=?"
				+ " AND s.DocStatus = 'CO'"
				+ " AND o.ov_ocmcerrada = 'N'"
				+ " AND (sl.Qty-sl.QtyDelivered) > 0"
				+ " AND s.AD_Org_ID="+ getAD_Org_ID()
				+ " GROUP BY s.OV_Llegada_ID,s.DocumentNo,s.DateDoc");
		try
		{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		/**faaguilar OFB
		 * despachos parciales begin*/
		 if(forInvoice) 
			    sql.append("l.QtyOrdered-SUM(COALESCE(invl.QTYINVOICED,0))," ); 
			    else 
			    sql.append("l.QtyOrdered-SUM(COALESCE(inout.MovementQty,0))," ); 
		/**despachos parciales end*/
		
				///+ "l.QtyOrdered-SUM(COALESCE(m.Qty,0)),"					//	1  faaguilar OFB original code commented
		sql.append("CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END,"	//	2
				...
				+ " l.C_OrderLine_ID,l.Line,l.datepromised,l.description "	//	10..11 //faaguilar OFB add l.datepromised
				+ "FROM C_OrderLine l"
		...
		sql.append(" IS NOT NULL)")*///faaguilar OFB commented original code
		...
		sql.append(" WHERE l.C_Order_ID=? "			//	#1
				...
				+ "l.M_Product_ID,COALESCE(p.Name,c.Name), l.Line,l.C_OrderLine_ID,l.datepromised,l.description " //faaguilar OFB add l.datepromised
				+ "ORDER BY l.Line");
		//
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if(qtyEntered.equals(Env.ZERO) && !isRangeActive()) //faaguilar OFB
					continue;
				
				line.add(qtyEntered);  //  1-Qty
				KeyNamePair pp = new KeyNamePair(rs.getInt(3), rs.getString(4).trim());
				line.add(pp);                           //  2-UOM
				line.add(rs.getTimestamp(12)); //3-datepromised
				// Add locator
				...
				line.add(rs.getString("description")); // 9 descripcion faaguilar OFB
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected Vector<Vector<Object>> getRMAData(int M_RMA_ID)
	{
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - rl.QtyDelivered, iol.M_Product_ID, p.Name, uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name), rl.created ");//faaguilar OFB add created
	    ...
	    sqlStmt.append("SELECT rl.M_RMALine_ID, rl.line, rl.Qty - rl.QtyDelivered, 0, c.Name, uom.C_UOM_ID, COALESCE(uom.UOMSymbol,uom.Name), rl.created ");//faaguilar OFB add created
	    ...
	    try
	    {
	        ...
	        while (rs.next())
            {
	            ...
	            line.add(pp); // 2-UOM
	            line.add(rs.getTimestamp("created"));//3 - created faaguilar OFB
				line.add(getLocatorKeyNamePair(0));// 4-  locator
	...
	protected Vector<Vector<Object>> getInvoiceData(int C_Invoice_ID)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT " // Entered UOM
				...
				+ " l.C_OrderLine_ID, inv.DateInvoiced,l.description " //faaguilar OFB add inv.DateInvoiced
		...
		.append(" LEFT OUTER JOIN M_InOutLine inoutl ON (l.C_InvoiceLine_ID=inoutl.C_InvoiceLine_ID)") //faaguilar OFB
		...
		.append("GROUP BY l.QtyInvoiced,l.QtyEntered/l.QtyInvoiced,"
				...
				+ "l.M_Product_ID,p.Name, po.VendorProductNo, l.C_InvoiceLine_ID,l.Line,l.C_OrderLine_ID,inv.DateInvoiced,l.description ")//faaguilar OFB add inv.DateInvoiced
		...
		try
		{
			...
			while (rs.next())
			{
				...
				KeyNamePair pp = new KeyNamePair(rs.getInt(3), rs.getString(4).trim());
				line.add(pp); // 2-UOM
				line.add(rs.getTimestamp(13)); //3-datepromised //faaguilar OFB add
				// Add locator
				...
				line.add(rs.getString("description")); // 9 descripcion faaguilar OFB
				line.add(null); // 9 descripcion faaguilar OFB
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected Vector<Vector<Object>> getLlegadaData(int OV_Llegada_ID)
	{
		...
		p_llegada = new MLlegada(Env.getCtx(), OV_Llegada_ID, null);      //  save

		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		StringBuffer sql = new StringBuffer("SELECT ");
		sql.append("(l.Qty-l.QtyDelivered) Qty, l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"
				+ " p.M_Locator_ID, loc.Value, "
				+ " COALESCE(l.M_Product_ID,0),p.Name, "
				+ " po.VendorProductNo, "
				+ " l.OV_LlegadaLine_ID,l.Line,le.datedoc,l.description "
				+ " FROM OV_LlegadaLine l"
				+ " INNER JOIN OV_Llegada le ON le.OV_Llegada_ID = l.OV_Llegada_ID"
				+ " LEFT OUTER JOIN M_Product_PO po ON (l.M_Product_ID = po.M_Product_ID AND le.C_BPartner_ID = po.C_BPartner_ID) "
				+ " LEFT OUTER JOIN M_Product p ON (l.M_Product_ID=p.M_Product_ID)"
				+ " LEFT OUTER JOIN M_Locator loc ON (p.M_Locator_ID=loc.M_Locator_ID)");
		if (Env.isBaseLanguage(Env.getCtx(), "C_UOM"))
			sql.append(" LEFT OUTER JOIN C_UOM uom ON (l.C_UOM_ID=uom.C_UOM_ID)");
			...
			.append(Env.getAD_Language(Env.getCtx())).append("')");
		//
		sql.append(" WHERE l.OV_Llegada_ID=? "			//	#1
				+ "AND (l.Qty-l.QtyDelivered) > 0 "
				+ "GROUP BY (l.Qty-l.QtyDelivered), "
				+ "l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name), p.M_Locator_ID, loc.Value, po.VendorProductNo, "
				+ "l.M_Product_ID,p.Name, l.Line,l.OV_LlegadaLine_ID,le.datedoc,l.description "
				+ "ORDER BY l.Line");
		//
		log.finer(sql.toString());
		PreparedStatement pstmt = null;
		...
		try
		{
			...
			while (rs.next())
			{
				Vector<Object> line = new Vector<Object>();
				line.add(new Boolean(false));           //  0-Selection
				BigDecimal qtyEntered = rs.getBigDecimal(1);
				line.add(qtyEntered);  //  1-Qty
				KeyNamePair pp = new KeyNamePair(rs.getInt(2), rs.getString(3).trim());
				line.add(pp);                           //  2-UOM
				line.add(rs.getTimestamp(11)); //3-datepromised
				line.add(getLocatorKeyNamePair(rs.getInt(4)));// 4-Locator
				pp = new KeyNamePair(rs.getInt(6), rs.getString(7));
				line.add(pp);                           //  4-Product
				line.add(rs.getString(8));				// 5-VendorProductNo
				line.add(null);                         //  7-Order
				line.add(null);                         //  8-Ship
				line.add(null);                         //  9-Invoice
				line.add(rs.getString("description")); // 10 descripcion
	...
	protected KeyNamePair getLocatorKeyNamePairByValue(String value)
	{
		MLocator locator = null;
		...
		if (value != null && value != "")
		{
	        ...
	        whereClause.append("Value='"+value+"'");
			locator = MLocator.get(Env.getCtx(), MLocator.getAllIDs(MLocator.Table_Name, whereClause.toString(), null)[0]);
			// Validate warehouse
			if (locator != null && locator.getM_Warehouse_ID() != getM_Warehouse_ID())
			{
				locator = null;
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(3, Timestamp.class, true);        //  3-datepromised //faaguilar OFB
		...
		miniTable.setColumnClass(10, String.class, true);   //  Description faaguilar OFB
		miniTable.setColumnClass(11, String.class, true);   //  Llegada
		...
		*/
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				if (miniTable.getValueAt(i, 4).getClass().getSimpleName().equals("String"))
					pp = getLocatorKeyNamePairByValue((String) miniTable.getValueAt(i, 4));
				else
					pp = (KeyNamePair) miniTable.getValueAt(i, 4); // Locator
				...
				//M_Locator_ID = defaultLocator_ID!=0?defaultLocator_ID:pp.getKey();//faaguilar OFB
				...
				pp = (KeyNamePair)miniTable.getValueAt(i, 11);               //  LlegadaLine
				if (pp != null)
					OV_LlegadaLine_ID = pp.getKey();
				
				//
//				MLlegadaLine llegadaLine = null;
//				if (OV_LlegadaLine_ID > 0)
//					llegadaLine = new MLlegadaLine(Env.getCtx(), OV_LlegadaLine_ID, null);
				//	Precision of Qty UOM
				
				// consulta si ubicacion tiene stock (llegada)
				BigDecimal disponible = null;
				try {
					String sql = "SELECT COUNT(*) AS disponible"
						+ " FROM M_Storage"
						+ " WHERE M_Locator_ID = " + M_Locator_ID;
					PreparedStatement pstmtps = DB.prepareStatement(sql, trxName);
					ResultSet rsps;
				...
				if (OV_LlegadaLine_ID != 0 && disponible != null && disponible.compareTo(BigDecimal.ZERO) > 0) {
					throw new IllegalArgumentException("Ubicación con stock.");
				}
				...
				/*faaguilar OFB comment begin , no negative, movement V-
				if (m_invoice != null && m_invoice.isCreditMemo() )
					QtyEntered = QtyEntered.negate();
				/*faaguilar OFB comment end , no negative, movement V- */
				
				//faaguilar OFB begin
				if (C_OrderLine_ID != 0){
					MOrderLine ol = new MOrderLine (Env.getCtx(), C_OrderLine_ID, trxName);
				   if(ol.getC_Charge_ID()>0)
					   if(ol.getC_Charge().getC_ChargeType_ID()>0)
					   if(ol.getC_Charge().getC_ChargeType().getValue().equals("TCAF"))//crear lineas por cada activo
					   {
						   for(int x=0;x<QtyEntered.intValue();x++){
				...
				if (OV_LlegadaLine_ID != 0)
				{	
					iol.set_ValueOfColumn("OV_LLEGADALINE_ID", OV_LlegadaLine_ID);
				}
		...
		if (p_llegada != null && p_llegada.getOV_Llegada_ID() != 0)
		{
			inout.set_ValueOfColumn("OV_LLEGADA_ID", p_llegada.getOV_Llegada_ID());
		}
		inout.saveEx();
		return true;		
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    columnNames.add("Fecha Prom"); //faaguilar OFB
	    ...
	    columnNames.add(Msg.getElement(Env.getCtx(), "Description", false));//faaguilar OFB
	...
	protected Vector<Vector<Object>> getLlegadaData (int OV_Llegada_ID, int M_Locator_ID)
	{
		defaultLocator_ID = M_Locator_ID;
		return getLlegadaData (OV_Llegada_ID);
	}
